...
(function() {
  var AsyncSoundManager, GameBoard, ProgressBar, SeedRandom, SinkTile, SourceTile, Splash, StartButton, Tile, TileGraphics, TubeTile, TubetasticGame, d, _ref, _ref1, _ref2,
  ...
  AsyncSoundManager = (function() {
    ...
    onSoundReady = function(evt) {
      sounds[evt.id] = evt;
    };

    createjs.Sound.addEventListener('fileload', createjs.proxy(onSoundReady, AsyncSoundManager));

    AsyncSoundManager.load = function(id) {
      var paths;
      if (!(id in sounds)) {
  ...
  TubeTile = (function(_super) {
    ...
    TubeTile.prototype.initialize = function(colNum, rowNum, x, y, s, board) {
      ...
      this.resize(x, y, s);
      this.ready = true;
      return AsyncSoundManager.load('sh');
    };
  ...
  ProgressBar = (function(_super) {
    ...
    ProgressBar.prototype.resize = function(w, h) {
      ...
      this.w = w;
      this.h = h;
      console.log('bar:', this.progress, this.w, this.h);
      padding = this.h / 8;
      this.removeAllChildren();
  ...
  Splash = (function(_super) {
    ...
    Splash.prototype.resize = function() {
      var credit, h, inst, progress, start, title1, title2, totalWidth, tw, w, y,
        _this = this;
  ...
  GameBoard = (function(_super) {
    ...
    GameBoard.prototype.resize = function() {
      ...
      this.tileSize = Math.floor(Math.min(this.stage.canvas.width / this.hopDepth, this.stage.canvas.height / this.sourceCount));
      this.x = Math.floor((this.stage.canvas.width - (this.tileSize * this.hopDepth)) / 2);
      this.y = Math.floor((this.stage.canvas.height - (this.tileSize * this.sourceCount)) / 2);
      console.log("board " + this.x + "," + this.y + " size:" + this.tileSize + " " + this.stage.canvas.width + "x" + this.stage.canvas.height);
      TileGraphics.resize();
      _ref3 = this.board;
      _results = [];
    ...
    GameBoard.prototype.powerSweep = function() {
      var colNum, direction, dropCount, id, neighbor, neither, onVanished, rowNum, sourced, sunk, sweepStart, tile, toCheck, toDrop, toRemove, toVanish, vanishCount, _i, _j, _k, _l, _len, _m, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9,
        _this = this;
      sweepStart = Date.now();
      this.ready = false;
      toCheck = [];
